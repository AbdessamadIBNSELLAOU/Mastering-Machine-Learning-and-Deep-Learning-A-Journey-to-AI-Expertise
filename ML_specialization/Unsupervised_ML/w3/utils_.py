...
def create_video(filename, env, q_network, fps=30):
    """
    Creates a video of an agent interacting with a Gym environment using a greedy policy.
    """
    with imageio.get_writer(filename, fps=fps) as video:
        ...
        state, _ = env.reset()
        frame = env.render()  # works if render_mode="rgb_array" was set when creating env
