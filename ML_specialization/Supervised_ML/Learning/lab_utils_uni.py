...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    # Conversion explicite en float64 pour éviter les erreurs d'overflow
    x_train = np.array(x_train, dtype=np.float64)
    y_train = np.array(y_train, dtype=np.float64)
    p_hist = np.array(p_hist, dtype=np.float64)
    J_hist = np.array(J_hist, dtype=np.float64)
    ...
    fix_b = 0  # peut être ajusté selon ton modèle
    w_array = np.arange(-50, 50, 1, dtype=np.float64)  # Réduction de l'échelle
    cost = np.zeros_like(w_array, dtype=np.float64)

    for i in range(len(w_array)):
        tmp_w = w_array[i]
        cost[i] = compute_cost(x_train, y_train, tmp_w, fix_b)

    ax.plot(w_array, cost, label='Cost curve')
    ax.plot(x, v, c=dlmagenta, label='Training path')
    ax.set_title("Cost vs w (b fixed)")
    ax.set_ylabel('Cost')
    ax.set_xlabel('w')
    ...
    tmp_b, tmp_w = np.meshgrid(
        np.arange(-50, 50, 2, dtype=np.float64),
        np.arange(-50, 50, 2, dtype=np.float64)
    )

    z = np.zeros_like(tmp_b, dtype=np.float64)
